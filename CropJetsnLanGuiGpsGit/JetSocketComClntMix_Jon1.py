...
def arwacmain(ArgAll):
    ...
    m_CLanParams.LaneOriPoint = src_vid['LaneOriPointL']#=50MainDeteTrack
    ...
    print("laneStartPo : {0}".format(m_CLanParams.laneStartPo)) 
    ...
    m_CLanParams.laneStartPo = m_CLanParams.laneStartPo
    ...
    # m_CLanParams.Numoflanes=Numoflanes#.FullDTHNumRowsInit  #4
    # m_CLanParams.laneoffset= laneoffset #25#25 #15  #25 
...
def run(killed):       
    ...
    msg_vec = list()
    svOutResPth= ''#'/media/dom/Elements/data/robctrl/'  

    N = 2000 # number of data points
    t = np.linspace(0, 4*np.pi, N)
    f = 1.15247 # Optional!! Advised not to use
    data = 0.8*np.sin(f*t+0.001)# + 0.5 + np.random.randn(N) # create artificial data with noise
    num_id = -1
    num_out = 0 # recording the number of output to Robots control
    sim_out = 0.0
    last_out = sim_out
    sleepT = 0.05
    ...
    while m_CamParams.key != 113 : #F2
        ...
        if m_CAgthmParams.issvo == True:
            ...
            m_CJetClnt.Data2Send(data[num_id])
            print('Dell7 sending out:', data[num_id])
            plt.pause(0.25)
            # continue
            """
            #############################
            if m_CamParams.cam.grab(m_CamParams.rt_param) != sl.ERROR_CODE.SUCCESS:
                status='filed to extract frame in .svo file'
                print(repr(status))
                break
            ...
            print ('Video time Running: {seTime} seconds for Steering:  {externa}'.format(seTime =secs, externa = sim_out))
            ...
            while secs < 3.0:
                ...
                # Sleep for a seconds
                m_CJetClnt.Data2Send(sim_out) 
                print(">>>>>>>>>>>>>>>>>>>>> {}".format(sim_out)) 
                time.sleep(sleepT) # don't sleep for a full second or else you'll be off
                # Increment the seconds in total
                # secs += sleepT
                secs = float((time.time() - start_time))
            """
            # secs = math.floor((time.time() - start_time))       
            # print("--- %s seconds ---" % (time.time() - start_time))         
            # print ('Time Taken: {seTime} To Steering:  {externa}'.format(seTime =secs, externa = sim_out))
            # time.sleep(0.125)
            # plt.pause(0.5)
            """            
            num_out = num_out+1
            if num_out%30==0:
                detec_data = [num_out] # recording the number of output to Robots control
                # list_of_elem = [Detec_Err1,Detec_Err3]
                detec_data.append(msg_scl)
                Errfilepth = svOutResPth+'rbctl'+'.csv'
                append_list_as_row(Errfilepth, detec_data)
            """
        else:
