...
from tools.FullRGBLaneDeteTrackGPs_cali import CFullRGBLneDetsTracks
# from tools.FullRGBLaneDeteTrack_mix import CFullRGBLneDetsTracks
from argparse import ArgumentParser as ArgParse
import os
from tools.CSysParamsGPs_Cali import CLanesParams,CAlgrthmParams,CCamsParams#,CYoloV3Params
# from CInitialVideoSettings_lbl import CVideoSrcSetting,CCameraParamsSetting,CRobtoParamSetting
from tools.CInitialVideoSettingsGpsLive import CVideoSrcSetting,CCameraParamsSetting,CRobtoParamSetting
...
def arwacmain(ArgAll):
    ...
    if m_CLanParams.issvo == True or  m_CLanParams.isavi == True:
        ...
        src_vid = m_VideoSrcSet.data31082021Hist_gps() # Hist Robot's farm , puddles, weather variant in middle
    else:
    ...
    if zed.is_opened()==True:
        zed.close()
    # Open the camera
    # if m_CLanParams.issvo ==False and m_CLanParams.isavi == False:
    err = zed.open(init)
...
def run(killed):       
    ...
    ctrFlag = 3 # selection of methods
    ...
    while m_CamParams.key != 113 : #F2
        ...
        if m_CLanParams.issvo == True or m_CLanParams.isavi ==True:
            ...
            if m_CAgthmParams.frame_num%25!=0:
                continue
            # Retrieve SVO images
            ...
            # msg['R'] = 999.0
            # msg['N'] = 0.0
            msg = sum(m_CLanParams.Ctrl_Ofset.values())
            m_CJetClnt.Data2Send(msg)
            plt.pause(0.25)            
            # cv2.namedWindow("Windwow")
            cv2.imshow("Windwow", m_CAgthmParams.video_image)
            key = cv2.waitKey(1)
        ...
        if err == m_CamParams.sl.ERROR_CODE.SUCCESS :
            ...
            m_CamParams.image_ocv = m_CamParams.left_image.get_data()
            m_CAgthmParams.frame_num=m_CAgthmParams.frame_num+1
            if m_CAgthmParams.frame_num<10:#
                continue
            # plt.close()
            ...
            # plt.title('Visible and Depth')        
            # plt.imshow(m_CamParams.image_ocv,cmap='gray')
            # plt.show(block=True)
            # plt.pause(0.25)
            # plt.close()
            ...
            m_CamParams.image_ocv = cv2.cvtColor(orig_frame, cv2.COLOR_RGB2BGR)


            # RGBPiplines(m_CamParams.image_ocv) 
            m_CFRGBLneTrcks.MainDeteTrack(m_CamParams.image_ocv) 

            msgi = m_CLanParams.Rbt_Ofset  # lane offset convert to control signal--- kind of look-ahead distance, or lateral offset
