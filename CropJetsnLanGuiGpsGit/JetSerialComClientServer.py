...
from RPi.SerialXComInterface4All import SerialComm#serial_ports, startSerialCom,runMotor
# from RPi.SerialXComInterface4AllThread import SerialCommObj
import enum
import ntpath
import time
import codecs
...
def arwacmain(ArgAll):
    ...
    # testing serial communication between PC and Jestson TX2, later on is Tx2  sending data to Rasberriy PI
    
    port='/dev/ttyUSB0' #ttyUSBACM0'
    baudrate=9600#115200   # 9600
    timeout = 3.0
    # serial_handle =SerialCommObj(port,baudrate)  #     
    serial_handle =SerialComm(port,baudrate, timeout)  # get instance of serial commmunication interface
    pwm1=50
    pwm2=50
    # serialport_vec = serial_handle.serial_ports(port,baudrate)
    # serialport_num = len(serialport_vec)
    """    # permission:
        ...
        https://forums.linuxmint.com/viewtopic.php?t=270846
         # temperory:
    ...
    """
    # print ('Number of serail port available: ', serial_handle.serialport_num)
    if serial_handle.serialport_num>0:
        # ser.port = "COM{}".format(COMPORT-1)
        # port = serialport_vec[0]
        ser=serial_handle.startSerialCom(port, baudrate,timeout)    
        if (ser):
            serial_handle.DPCWritePort(ser,0,pwm1,pwm2)

            # ser.open()
            if ser.isOpen():
                print ('PC side is ready! Enter your commands below.\r\nInsert "exit" or "off" to leave the application.')
            ...
            while True :
                ...
                # Python 3 users
                data = ''# input(">>  ")
                if data == 'exit' or data == 'off':
                    print("Existing ...")
                    time.sleep(0.1) 
                    # serial_handle.DPCWritePort(ser,'off',data,pwm2)  
                    ser.close()
                    exit()
                else:
                    ...
                    # serial_handle.DPCWritePort(ser,'pc',data,pwm2)
                    time.sleep(0.5)   
                    out = ''
                    # let's wait one second before reading output (let's give device time to answer)
                    
                    """
                    rev_buffer = serial_handle.read_Data(ser)
                    # rev_buffer.decode(encoding)
                    # read_buffer.decode('utf-8').strip()
                    read_buffer = rev_buffer.decode('utf-8')
                    revdata = codecs.decode(rev_buffer, 'UTF-8')
                    # revdata = str(rev_buffer, 'utf-8')
                    # revdata = str(read_buffer)
                    # revdata = np.str(read_buffer)
                    # read_buffer = out.join(map(chr, read_buffer))
                    print('Recieved data : ', revdata)
                    print ("Recieving read_buffer: {rev_buffer}".format(rev_buffer=read_buffer))
                    ...
                    while ser.inWaiting() > 0:
                        ...
                        data_in = ser.read(8)  # set 1 is one by one
                        if len(data_in) > 0:
        ...
        print('Checking Prot Number: ', serial_handle.serialport_num) 
