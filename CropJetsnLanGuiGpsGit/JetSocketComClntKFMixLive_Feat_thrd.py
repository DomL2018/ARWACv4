...
from tools.KFcvFeatMixLaneDeteTrack import CKFcvFeatLneDetsTracks
...
from ARWACv2dom.mavlink_ctrl_thrd import CMavlinkCtrl
...
def main():
    ...
    # m_MavlinkCtrl = CMav?linkCtrl()  # temporitly for testing
    # 
    m_CLanParams=CLanesParams() 
    m_MavlinkCtrl = CMavlinkCtrl(m_CLanParams)
    ...
    # src_vid = m_VideoSrcSet.data_live_setting() # data 20 in paper, but 3 in collection, with black anther data set
    ...
    if m_CLanParams.issvo ==True or m_CLanParams.isavi == True:
        init_params.set_from_svo_file(str(m_CLanParams.video_fille_l))
    ...
    m_CKFFeatLneTrcks=  CKFcvFeatLneDetsTracks(m_CAgthmParams,m_CLanParams)
    m_CLanParams.mt,m_CLanParams.mt_inv = m_CKFFeatLneTrcks.perspective_transform((m_CLanParams.w0,m_CLanParams.h0),m_CLanParams.laneoffset)
...
def start_zedthrd():
    ...
    stop_threads = False
    zed_callback = threading.Thread(target=run,args =(lambda : stop_threads,))
    zed_callback.start()
    mav_callback = threading.Thread(target=m_MavlinkCtrl.run, args=(m_CLanParams.msg2ctr, m_CLanParams.msg2ang))
    mav_callback.start()

    zed_callback.join()  # gurantee to finish execution, but may not be this case inside loop
    mav_callback.join()  # https://wellsr.com/python/python-multithreading-run-functions-in-parallel/
...
def run(killed):       
    ...
    while m_CamParams.key != 113 : #F2: 
        ...
        if m_CLanParams.issvo == True or m_CLanParams.isavi ==True:
            ...
            m_CKFFeatLneTrcks.MainDeteTrack(orig_frame) 
            # msg['L'] = 555.0
            # msg['R'] = 999.0
            # msg['N'] = 0.0
        ...
        if err == m_CamParams.sl.ERROR_CODE.SUCCESS:                      
            ...
            if m_CamParams.frame_num<3:#<=m_CLanParams.frm2strart:
                continue 
            m_CKFFeatLneTrcks.MainDeteTrack(orig_frame)#,'','',m_CLanParams.pf_start,'') 
            ...
            m_CLanParams.msg2ctr=sim_out
            m_CLanParams.msg2ang =m_CLanParams.msg2ang
            last_out = sim_out 
            time.sleep(sleepT)
            rb_sleepT = rb_sleepT+sleepT
            secs = round((time.perf_counter() - start_time),3)       
            # print("--- %s seconds ---" % (time.time() - start_time))         
            print ('\n Live time Running: {seTime} seconding :  {externa}'.format(seTime =secs, externa = sim_out))
