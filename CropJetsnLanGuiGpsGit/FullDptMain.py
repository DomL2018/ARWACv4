...
def run(killed):       
    ...
    while m_CamParams.key != 113 : #F2
        ...
        if err == m_CamParams.sl.ERROR_CODE.SUCCESS :
            ...
            plt.pause(0.25)
            plt.close()
            # zed-opencv: Retrieve the left image, depth image in the half-resolution ----------- only for the display purpose
            ...
            DepthMapPiplines(m_CamParams.image_ocv,m_CamParams.depth_image_ocv)
                # pass
...
# def RGBPiplines(img):   
    # plt.figure('ZED Camera Live')
    # plt.title('Visible and Depth')        
    # plt.imshow(img,cmap='gray')qq
    # plt.show(block=False)
    # plt.pause(0.25)
    # plt.close()
    # m_CFRGBLneTrcks.MainDeteTrack(img)
def DepthMapPiplines(img,dpth):
