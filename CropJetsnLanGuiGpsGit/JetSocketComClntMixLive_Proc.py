...
import math, enum
import ntpath,time
from ARWACv2dom.mavlink_ctrl_prc import CMavlinkCtrl
...
# too heavy to run zed with 2 backgound process, and not afford to this
# https://stackoverflow.com/questions/3474382/how-do-i-run-two-python-loops-concurrently/27524134
...
def start_zed_mav():
    ...
    # granted that we forgot to wait for the processes to finish but according to the output, 
    # the processes were started after the program finished execution. 
    # The output appears in this order because it takes a while to create the processes and get them running. This isn’t the case for threads that start instantly. 
    
    # Like threads, the join() method is used to wait for the processes to finish execution. 
    # https://analyticsindiamag.com/run-python-code-in-parallel-using-multiprocessing/
    zed_callback = Process(target=run,args =(lambda : stop_threads,))
    zed_callback.start()
    mav_callback = Process(target=m_MavlinkCtrl.run, args=(m_CLanParams.msg2ctr, m_CLanParams.msg2ang))
    mav_callback.start()
...
def run(killed):       
    ...
    while m_CamParams.key != 113 : #F2
        ...
        # using .svo files
        if m_CAgthmParams.issvo == True or m_CAgthmParams.isavi ==True:
        ...
        if err == m_CamParams.sl.ERROR_CODE.SUCCESS :
            ...
            m_CLanParams.msg2ang =m_CLanParams.msg2ang + 5.0
            last_out = sim_out 
            time.sleep(sleepT)
