...
class CFullRGBLneDetsTracks(CLanesParams):
    ...
    def MainDeteTrack(self,orig_frame):
        ...
        PltShowing(report,'warping - croped...','warped-snip', True, True)
        ...
        if self.LanesParams.firstFrame is False:
            ...
            for ikey, p in self.LanesParams.grasslanes.items():#.iteritems(): 
                ...
                # for the practical offset to control the hose manoveoveor

                self.LanesParams.Ctrl_Ofset[index]=ikey-p.id
                # the absolute value to original image
                self.LanesParams.Ctrl_Ofset[index]=0.5*(self.LanesParams.image_size.width - snpwidth)+p.id
                # index='lane_'+str(ikey)
                # self.LanesParams.Lane_Detect_val[index]=p.id
        ...
        if nums_deted!=0:
            self.LanesParams.Rbt_Ofset = sum(self.LanesParams.Ctrl_Ofset.values())/nums_deted#*self.LanesParams.deltalane)
        else:
            self.LanesParams.Rbt_Ofset = sum(self.LanesParams.Ctrl_Ofset.values())/nums_caled#  (self.LanesParams.Numoflanes)#*self.LanesParams.deltalane)
