...
class CFullRGBLneDetsTracks(CLanesParams):
    ...
    def CameraStreamfromfile(self):
        ...
        print('ocv: ={0}'.format(self.LanesParams.video_ocv))
        try:
            ...
            if not(self.LanesParams.video_ocv):
                ...
                if (self.LanesParams.camera.isOpened()== False): 
                    ...
                    print('frame total numer={0}, width={1}, Height={2}'.format(self.LanesParams.totalNum_frame,self.LanesParams.frame_width,self.AgthmParams.frame_height))
            ...
            self.LanesParams.camera = cv2.VideoCapture(0)
    ...
    def MainDeteTrack(self,orig_frame):
        ...
        if self.LanesParams.isavi ==True:     
            ...
            if self.LanesParams.frame_num == self.LanesParams.totalNum_frame-1:
                # recording_counter=recording_counter+1
                self.LanesParams.camera = cv2.VideoCapture(self.LanesParams.video_ocv)
            grabbed, orig_frame = self.LanesParams.camera.read()
            while (self.LanesParams.frame_num<self.LanesParams.frm2strart):
                self.LanesParams.frame_num+=1
                grabbed, orig_frame = self.LanesParams.camera.read()            
                continue        

            if (self.AgthmParams.frame_num>self.LanesParams.video_counter or grabbed is False):
        ...
        PltShowing(masked,'detect...','line segements',True,False)
        ...
        if self.LanesParams.firstFrame is True:
            ...
            for la in range(laneStartPo,snpwidth-laneStartPo+1,lanegap):           
                ...
                self.LanesParams.grasslanes[la]=grasstrack(la,masked,track_pt,(x,y,w,h),[],lanebandwith,deltalane)
                index='lane_'+str(la)
        ...
        cent_refined = []
        for ikey, p in self.LanesParams.grasslanes.items():
        ...
        # PltShowing(res,'Lane tracking...','Perspective View Tracks',False, True)
