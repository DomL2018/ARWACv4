...
from tools.CSysParamsAll import CLanesParams#,CAlgrthmParams,CCamsParams
...
class CFullRGBLneDetsTracks1(CLanesParams):
    ...
    def get_lines(self,regionofintrest,masked):    
        ...
        if lines is not None:
            for line in lines:
                ...
                if (abs(math.degrees(orient)) > 45.0) and abs(math.degrees(orient)) < (135.0):
    ...
    def get_dic(self,cent_refined,_lines_mid_v,masked,bandwith):   
        ...
        # return [l[0] for l in lines_in]
    # illustration only
    def Display_Info_Panel(self,orig_frame):
        ...
        #
        """
        # lane_color[0:m_CLanParams.h0, m_CLanParams.image_size.width-m_CLanParams.w0:m_CLanParams.image_size.width] = m_CLanParams.video_image
        orig_frame[0:self.LanesParams.h0, self.LanesParams.image_size.width-self.LanesParams.w0:self.LanesParams.image_size.width] = self.LanesParams.video_image

        # road_info, curvature, deviation = get_measurements(left_line, right_line)
        cv2.putText(orig_frame, 'Detecting & Control ', (75, 30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 0, 0), 4)

        lane_info = 'Total Number of Lanes: ' + str(self.LanesParams.Numoflanes)             
        interx= 10
        intery = 90   
        cv2.putText(orig_frame, lane_info, (interx, intery), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 0, 0), 4)        
        intery = intery+30
        # if curvature == -1:
        ...
        tmp = np.array(list(self.LanesParams.Ctrl_Ofset.values())).mean()
        self.LanesParams.hose_offset = round(np.array(list(self.LanesParams.Ctrl_Ofset.values())).mean(),2)

        if tmp<0:
            hose_offset_str = 'Hose Moving To - Left (cm): '+str(self.LanesParams.hose_offset)
        else:
            hose_offset_str = 'Hose Moving To - Right (cm) '+str(self.LanesParams.hose_offset)
        cv2.putText(orig_frame, hose_offset_str, (interx, intery), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 0, 0), 2)
        intery = intery+30
        strtmp =self.LanesParams.Lane_GTstr + ' - Pixels in Detecting Window: '
        cv2.putText(orig_frame, strtmp, (interx, intery), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 0, 0), 2)

        for key, value in self.LanesParams.Ctrl_Ofset.items():
            intery = intery+30
            strtmp = key + ' Offset (pix): '+ str(round(value,2))
            cv2.putText(orig_frame, strtmp, (interx, intery), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0, 0, 0), 2)
    ...
    def draw_display(self,masked,lane_dic,cen_hight,width):
        ...
        PltShowing(masked,'draw-display...','detected lane',False,False)
        return masked
    ...
    def MainDeteTrack(self,orig_frame):
        ...
        # m_CLanParams.mt,m_CLanParams.mt_inv
        """img_size = (orig_frame .shape[1], orig_frame.shape[0])
        self.LanesParams.ori_warped = cv2.warpPerspective(orig_frame, self.LanesParams.mt, img_size, flags=cv2.INTER_LINEAR)
        report = np.hstack((self.LanesParams.ori_warped, self.LanesParams.orig_frame)) #stacking images side-by-side 
        
        PltShowing(report,'Camera Output...','Warped and Origin', True,True)
        warped = self.LanesParams.ori_warped[row0:row0+h0,col0:col0+w0]"""
        ...
        PltShowing(report,'warping - croped...','warped-snip', False, False)
        ...
        # how many lanes detected actually
        nums_deted = len(lane_dic.items())
        ...
        if self.LanesParams.firstFrame is True:
            ...
            for la in range(laneStartPo,snpwidth-laneStartPo+1,self.LanesParams.deltalane):           
                ...
                self.LanesParams.Lane_GTstr = self.LanesParams.Lane_GTstr + ' '+ str(la)
        ...
        if self.LanesParams.firstFrame is False:
            ...
            for ikey, p in self.LanesParams.grasslanes.items():#.iteritems(): 
                ...
                for key, coords in lane_dic.items(): # try to get each column
                    ...
                    #     #    detect = p.update(detect,track_pt,(x,y,w,h))
                       ...
                       if xpos > width:
                           ...
                           xpos = 4
                       detect = cv2.line(detect,(xpos,height-1),(xpos,0),(255,255,0),8)                       
                       lane_dic.pop(key)
                       found_flg=True
                       break
                
                if found_flg==False:
                    print("Lane id: %d is not updated!" % p.id)
                ...
                pre_updated_id = p.id  # added for this purpose only - 14/09
                index='lane_'+str(ikey)
                self.LanesParams.Lane_Detect_val[index]=p.id

                # for the practical offset to control the hose manoveoveor using below
                # pixelSize = self.LanesParams.PixelSize
                self.LanesParams.Ctrl_Ofset[index]=round((ikey-p.id)*self.LanesParams.PixelSize,3)
                # the absolute position to original image for demonstrating using below
                self.LanesParams.Ctrl_OfsetPos[index]=round(0.5*(self.LanesParams.image_size.width - snpwidth)+p.id,3)
                # index='lane_'+str(ikey)
                # self.LanesParams.Lane_Detect_val[index]=p.id
        ...
        nums_caled = len(self.LanesParams.Ctrl_Ofset)
        # unwarped = cv2.warpPerspective(masked, self.LanesParams.mt_inv, (masked.shape[1], masked.shape[0]), flags=cv2.INTER_LINEAR)  # DEBUG
        # unwarped_snip = cv2.warpPerspective(detect, self.LanesParams.mt_inv, (detect.shape[1], detect.shape[0]), flags=cv2.INTER_LINEAR)  # DEBUG
        ...
        self.LanesParams.video_image = detect#plt.imshow(self.LanesParams.video_image)
        # plt.pause(.5)
        # plt.draw()        
        # plt.close()
