...
def shadow_remove1(img):
    ...
    for plane in rgb_planes:
        ...
        res_planes.append(diff_img)
        res_norm_planes.append(norm_img)

    shadowremov = cv2.merge(res_planes)
    shadowremov_norm = cv2.merge(res_norm_planes)
    return shadowremov, shadowremov_norm
...
def RGBColorSegmt(src):
    ...
    thresh,bin_img = cv2.threshold(blur,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    # (thresh, bin_img) = cv2.threshold(gray_u8, -1.0, 255, cv2.THRESH_OTSU)
    # cv2.imshow('bin_img', bin_img)
    ######################################
    PltShowing(bin_img,'Segmt...','Binaryd', True, True)
    ...
    PltShowing(blurred,'Tracking...','Blurred', True, True)
    ...
    PltShowing(res,'Tracking...','erosion - dilation',  True, True)
    ...
    # blurred=erosion#erosion#
    edges = cv2.Canny(blurred, 50, 200) #150
    res = np.hstack((blurred,edges))
    PltShowing(res,'Segmentation...','blured - edges', True, True)
    return edges, erosion, bin_img
...
def RGBColorSegmtFull(image,name_pref):
    """
    ...
    # listimages.append(sobelxedges)
    # listtitles.append('sobelxedges')

    sobelyedges = cv2.Canny(sobely, minthres, maxtres) #150
    # listimages.append(sobelyedges)
    # listtitles.append('sobelyedges')
    
    green_wheat = cv2.bitwise_and(warped, warped, mask=sobelyedges) 
    # listimages.append(green_wheat)
    # listtitles.append('green_wheat')
    ############################################
    ############################################
    ...
    # green_wheat = np.copy(green_MEAN)
    # green_wheat = np.copy(green_GAUSSIAN)
    ...
    # green_wheat = np.copy(green_OTSU)
    return green_wheat,sobelyedges
##########################################################
def HSVFullEdgeProcSeg(image,lowerLimit,upperLimit,name_pref=''):
    """
    a compex process for heavy duty
    added more pre-processing operation, this can be focuing on the heavy cannopy and overlapping images
    """    
    listimages = [image]
    listtitles = ["Warped"]
    ...
    thrimage = opening#erosion#cdilation#erosion#closing#opening
    blcksiz = 5
    bordsize = 2
    maxval =225
    ...
    listtitles.append('adapt OTSU-th1')
    listimages.append(green_OTSU)
    listtitles.append('Overlapped with th1')


    th2 = cv2.adaptiveThreshold(thrimage,maxval,cv2.ADAPTIVE_THRESH_MEAN_C,\
        cv2.THRESH_BINARY,blcksiz,bordsize)
    green_MEAN = cv2.bitwise_and(warped, warped, mask=~th2)  # for distinctivity
    listimages.append(th2)
    listtitles.append('adapt MEANC-th2')
    listimages.append(green_MEAN)
    listtitles.append('Overlapped with th2')
    # plt.gcf().suptitle('MEAN_C dim: ' + str(blcksiz) +
                        # '\nbordsize: ' + str(bordsize))
    ...
    green_GAUSSIAN = cv2.bitwise_and(warped, warped, mask=th3) # for distinctivity    
    listimages.append(th3)
    listtitles.append('adapt GAUSSC-th3')
    listimages.append(green_GAUSSIAN)
    listtitles.append('Overllaped with th3')
    # plt.gcf().suptitle('GAUSSIAN_C dim: ' + str(blcksiz) +
                        # '\nbordsize: ' + str(bordsize))
...
def HSVCmplImProc(image,lowerLimit,upperLimit,name_pref):
    """
    a compex process for heavy duty
    added more pre-processing operation, this can be focuing on the heavy cannopy and overlapping images
    """    
    ...
    return green_wheat
        ...
        #######################################
def PltShowing(img,figname,titlname, pauseflg=True, dipFlag = True):
    # return
    # pauseflg=True
    # dipFlag = True
    # pauseflg=False
...
def cvSaveImages(img,figname='xx.jpg',path='./', svflg=False, subpth = '',extname='.jpg', covFlg = True):
    if svflg==True:
...
def cvSaveImages4Shdw(img,figname='xx', flg=False,path='./',extname='.jpg'):
    # return
    if flg==True:
        # path = '/media/dom/Elements/data/data_collection/data19112020/'
        path = '/media/dom/Elements/data/shadw_prc/'
        # path = '/media/dom/Elements/data/data_collection/data26112020/'
        cv2.imwrite(path+figname+extname,cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    else:
        return


def HSVSimpleImProc(image,low_green,high_green):
    # a light prcess on image basic operation, which apply for simple environment
    # image = cv2.imread(r'Image.jpg') #load your image
    warped = np.copy(image)
    snip = np.copy(image)
    hsv_frame = cv2.cvtColor(snip, cv2.COLOR_BGR2HSV) #convert the image into hsv
    res = np.hstack((snip,hsv_frame)) #stacking images side-by-side
    PltShowing(res,'Tracking...','RGB & HSV', True,True)
    cvSaveImages(res,'Fig11WheatRGB2HSVHst.jpg')

    hsv_green_mask = cv2.inRange(hsv_frame, low_green, high_green)
    PltShowing(hsv_green_mask,'Tracking...','hsv_green_mask', True)

    #######################################
    normalizedImg = cv2.normalize(hsv_green_mask,  hsv_green_mask, 0, 255, cv2.NORM_MINMAX)
    PltShowing(normalizedImg,'Tracking...','Normalized hsv_green_mask', True)

    greenInOriframe = cv2.bitwise_and(snip, snip, mask=normalizedImg)
    PltShowing(greenInOriframe,'Tracking...','Crops Region Extraction', True)

    print ("hsv_green_mask and greenInOriframe dimension:", hsv_green_mask.shape, greenInOriframe.shape)
    stacked_hsv_3d = np.stack((hsv_green_mask,)*3, axis=-1)
    res = np.hstack((stacked_hsv_3d,greenInOriframe)) #stacking images side-by-side
    PltShowing(res,'Tracking...','Green space HSV and RGB image', True)
    #########################################
    # Create our shapening kernel, it must equal to one eventually
    # kernel_sharpening = np.array([[-1,-1,-1], 
    #                           [-1, 9,-1],
    #                           [-1,-1,-1]])
    # hsv_green_mask = cv2.filter2D(hsv_green_mask, -1, kernel_sharpening)
    equ = cv2.equalizeHist(normalizedImg)
    # applying the sharpening kernel to the input image & displaying it.
    ...
    sgmspace=75
    blurred = cv2.bilateralFilter(hsv_green_mask,dims,sgmcolor,sgmspace)
    PltShowing(blurred,'Tracking...','bilateralFilter', True)
    cvSaveImages(blurred,'Fig12WheatBilateralHst.jpg')
    # morphological operation kernal - isolate the row - the only purpose
    # small kernel has thining segmentation , which may help with later hough transform, then group the segments of line
    #https://github.com/opencv/opencv/blob/master/samples/python/tutorial_code/imgProc/morph_lines_detection/morph_lines_detection.py
    # 1st erode then dialted , in most case like this :   erode - dilate in above link
   ...
   #####################################################
    ...
    # Closing  = Dilation followed by Erosion
    closing = cv2.morphologyEx(blurred, cv2.MORPH_CLOSE, kernel)
    # blurred=erosion # erosion,dilation #closing
    green = cv2.bitwise_and(warped, warped, mask=blurred)
    PltShowing(green,'ROI','Extracted Green',True)
    
    edges = cv2.Canny(blurred, 50, 200) #150
    res = np.hstack((blurred,edges)) #stacking images side-by-side
    PltShowing(res,'Tracking...','Input - Edges',True)
    cvSaveImages(res,'Fig22WheatLineEdgesHst.jpg')

    fig = plt.figure('morphological operation', figsize=(36, 24), dpi=80, facecolor='w', edgecolor='k')
    fig.canvas.set_window_title('Various Mophological Operation')
    ax = fig.add_subplot(221)
    ax.title.set_text('Erosion')
    plt.subplot(2, 2, 1)
    plt.imshow(cv2.cvtColor(erosion, cv2.COLOR_BGR2RGB) , cmap="gray")

    ax = fig.add_subplot(222)
    ax.title.set_text('Dilation')   
    plt.subplot(2, 2, 2)
    plt.imshow(cv2.cvtColor(dilation, cv2.COLOR_BGR2RGB) , cmap="gray") 

    ax = fig.add_subplot(223)  
    ax.title.set_text('Opening')   
    plt.subplot(2, 2, 3)
    plt.imshow(cv2.cvtColor(opening, cv2.COLOR_BGR2RGB) , cmap="gray")  

    ax = fig.add_subplot(224)  
    ax.title.set_text('Closing')   
    plt.subplot(2, 2, 4)
    plt.imshow(cv2.cvtColor(closing, cv2.COLOR_BGR2RGB) , cmap="gray")
    plt.show(block=False)
    plt.close()
