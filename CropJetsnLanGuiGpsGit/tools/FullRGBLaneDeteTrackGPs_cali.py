...
from tools.CSysParamsGPs_Cali import CLanesParams,CAlgrthmParams,CCamsParams
...
class CFullRGBLneDetsTracks(CAlgrthmParams,CLanesParams):
    ...
    def get_lines(self,regionofintrest,masked):    
        ...
        PltShowing(masked,'Tracking...','line segments',False,True)
    ...
    def MainDeteTrack(self,orig_frame):
        ...
        PltShowing(orig_frame,'Camera Output...','Origin', True,True)

        snip = orig_frame[row0:row0+h0,col0:col0+w0]
        PltShowing(snip,'Camera Output...','snip', True,True)
        ...
        PltShowing(warped,'warping...','warped', True, True)
        ...
        """
        self.AgthmParams.video_image = np.hstack((detect,masked)) #stacking images side-by-side
        """        
        self.AgthmParams.video_image = plt.imshow(self.AgthmParams.video_image)
        plt.pause(.5)
        plt.draw()
        """
