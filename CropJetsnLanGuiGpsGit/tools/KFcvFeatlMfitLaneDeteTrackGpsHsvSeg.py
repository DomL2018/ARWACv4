...
from tools.HSVRGBSegt4HTSeg import PltShowing,cvSaveImages, RGBColorSegmt ,HSVFullEdgeProcSeg, RGBColorSegmtFull#, 
from tools.imagetranformLive import skim_ToplogicalSkel#,cv_ToplogicalSkel
...
class KFcvFeatlMfitLaneDeteTracks(CAlgrthmParams,CLanesParams):
    ...
    def centroid_refining_new(self,centroids,bandwith):
        ...
        return cent_refined
    #line segments detection by cvFeats , 
        ...
        for k in range(keys_len):#range(LanesParams.SIMPNumoflaneInit):
            ...
            for key, group in it.groupby(T, key_f):                    
                ...
                for k in range(keys_len):
                    ...
                    lanewidth =  0.618*self.AgthmParams.grasslanes[lanekeys[k]].lanegap
    ...
    #line segments detection by cvFeats , 
        # detect lines within binary image , and draw straight line within the image
    # no subwindow is to be used, only one detection, like in other project before.
    #lanes detection by meanshift/xmean clustering
    def to_matrix(self,l, n):  # 1d list to 2 d list
        return [l[i:i+n] for i in range(0, len(l), n)]

    def get_lines_seg(self,segment,masked):    

        # skinning the segment for thining lines direction and regressiogn ....
        skelcv, skelee,mask_ske =skim_ToplogicalSkel(segment,'/home/dom/Documents/ARWAC/robdata/lineSegs',64) # threshold = 64 changeable
        ret_lanes = self.find_lane_arwacline(masked,mask_ske)

        return ret_lanes

   
    #lanes detection by meanshift/xmean clustering
    def find_lane_arwacline(self,warped, img): 
        
        masked = np.stack((img,)*3, axis=-1)#(np.dstack((img, img,img))*255).astype('uint8')
        # meanshift:
        # print('datax',datax)
        #The following bandwidth can be automatically detected using    # 
        cen_hight = 0.618*img.shape[0]
        # print('bandwith_ested=',bandwith_ested)
        
        self.keys = list(self.AgthmParams.grasslanes)
        self.lengh = len(self.keys)
        # print('total number of lanes tracked = {0}'.format(self.lengh))
        lane_base = []
        lane_inds = {}
        for k in range(self.lengh):
            lane_base.append(self.keys[k])  #
            lane_inds[k] = []

        nonzero = img.nonzero()
        nonzeroy = np.array(nonzero[0])
        nonzerox = np.array(nonzero[1])
        # tmplate = np.zeros_like(masked)
        # tmplate = masked[nonzero]
        nonzeroy_lst = nonzeroy.tolist()
        nonzerox_lst = nonzerox.tolist()
        nonzeroy_2d =  self.to_matrix(nonzeroy_lst, 1)
        nonzerox_2d =  self.to_matrix(nonzerox_lst, 1)
        
        cent_refined,masked,bandwith_ested= self.get_lanes_mshfit(nonzerox_2d,nonzeroy_2d,[],masked,cen_hight,self.LanesParams.lanebandwith)
        _lines_mid_v = np.array((nonzerox_2d, nonzeroy_2d))
        _lines_mid_v = _lines_mid_v.T
        lane_dic= self.get_dic_mshfit(cent_refined,_lines_mid_v,masked,self.LanesParams.lanebandwith)
  
        ret = {}
        ret['cent_refined']=cent_refined
        ret['left_fit'] = [0,0,0]
        ret['right_fit'] = [0,0,0]
        ret['nonzerox'] = nonzerox_2d
        ret['nonzeroy'] = nonzeroy_2d
        ret['out_img'] = masked
        # ret['left_lane_inds'] = left_lane_inds
        # ret['right_lane_inds'] = right_lane_inds
        ret['lane_dic'] = lane_dic

        # self.viz2(img, ret, None)
        # ret = self.update_lane_crops(warped, ret)
        return ret                     
    def centroid_refining_new(self,centroids,bandwith):
        cen_lth = len(centroids)-1  # all list index number
        inx = 0
        ...
        return cent_refined
    #lanes detection by meanshift/xmean clustering
    def get_lanes_mshfit(self,datax,datay,_lines_mid_v,masked,cen_hight,bandwith): 
        
        bandwith_ested = self.LanesParams.lanebandwith
        if len(datax)<2:
            cent_refined = []
            return  cent_refined,masked   
        """ xmean :   
        ...
        ms = MeanShift(bandwidth=self.LanesParams.lanebandwith, bin_seeding=True)
        ms.fit(datax)
        labels = ms.labels_
        centroids = ms.cluster_centers_
        print('lanebandwith = {0}, centroids= {1}'.format(self.LanesParams.lanebandwith,centroids))
        ...
        #     cent_refined = centroid_refining(cent_refined,bandwith)
        #     if lenofrefined_prv==len(cent_refined):
        #         break

        cols = int(masked.shape[1])
        rows = int(masked.shape[0])
        print('lenth of centroids refined= ',len(centroids))
        if centroids is not None:
            for cent in centroids:
                ...
                masked = cv2.line(masked,(cx,rows-1),(cx,0),(200,0,200),5) 
    ...
    #lanes detection by meanshift/xmean clustering
    def get_lanes_dbscan(self,datax,datay,_lines_mid_v,masked,cen_hight,bandwith): 
        ...
        return  centroids,masked,bandwith_ested
        # return [l[0] for l in lines_in]

    

    def get_dic_mshfit(self,cent_refined,_lines_mid_v,masked,bandwith):   
    ...
    def MainDeteTrack(self,orig_frame):
        ...
        """
        self.AgthmParams.filter_start=self.AgthmParams.filter_start+1
        ...
        Pth_sve4paper = ''
        # im_2gbr,green_wheat = RGBColorSegmtFull(warped,'tx2')
        # using  HSV domain segmentation
        low_green = self.LanesParams.low_green #np.array([gclor_l, gSatur_l, gval_l])
        high_green = self.LanesParams.high_green  #np.array([gclor_h, gSatur_h, gval_h])
        regionofintrest = HSVFullEdgeProcSeg(warped,low_green,high_green,'Tx2') 



        # # usiing rgb sementation
        # edges, erosion, bin_img = RGBColorSegmt(warped)
        # # option 1:
        # regionofintrest = edges
        # # option 2:
        # regionofintrest = erosion
        # # option 3:
        # regionofintrest = bin_img
        ...
        if self.AgthmParams.firstFrame is True:
            ...
            for la in range(laneStartPo,snpwidth-laneStartPo+1,self.LanesParams.deltalane):           
                ...
                self.AgthmParams.grasslanes[la]=grasstrack(la,masked,track_pt,(x,y,w,h),[],lanebandwith,deltalane)
                index='lane_'+str(la)
        ...
        # regionofintrest,feat_img =self.AgthmParams.m_detector.get_cvFASTKeypoints(green_wheat)
        ...
        ret=self.get_lines_seg(regionofintrest,masked)
        ...
        # ret=self.get_lanes_seg(datax,datay,_lines_mid_v,masked,cen_hight,lanebandwith)      
        #get dictionary centroid <=> lane : (x,y)
        lane_dic= ret['lane_dic'] #self.get_dic_mshfit(cent_refined,_lines_mid_v,masked,lanebandwith)      
        ...
        if self.AgthmParams.firstFrame is False:
            ...
            for ikey, p in self.AgthmParams.grasslanes.items():#.iteritems(): 
                ...
                for key, coords in lane_dic.items(): # try to get each column
                    ...
                    #    if self.AgthmParams.filter_start>=self.AgthmParams.upd_thrsh: #abs(p.id-p.center[0])<(lanebandwith*0.1545):
                ...
                #     if self.AgthmParams.filter_start>self.AgthmParams.upd_thrsh:
                ...
                self.LanesParams.Ctrl_Ofset[index]= 0.5*(self.LanesParams.image_size.width - snpwidth)+p.id#pre_id-p.id
        ...
        cv2.imwrite('/home/dom/Documents/ARWAC/robdata/lineSegs/lineSegs.jpg',res)
        ...
        # self.AgthmParams.video_image = plt.imshow(res)
        # plt.pause(.1)
        # plt.draw()
