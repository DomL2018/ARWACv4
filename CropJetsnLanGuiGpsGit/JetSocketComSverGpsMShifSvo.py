...
from tools.FullRGBLaneDetHtMsGps import CFullRGBLneDetsTracks
...
def arwacmain(ArgAll):
   ...
   # Get image size
    ...
    width = m_CLanParams.image_size .width
    height = m_CLanParams.image_size .height
    # width_sbs = width * 2
    ...
    # m_CLanParams.mt,m_CLanParams.mt_inv = m_CFRGBLneTrcks.perspective_transform((width,height),m_CLanParams.laneoffset)
...
def run(killed):       
    ...
    while m_CLanParams.key != 113 : #F2
        ...
        if m_CLanParams.issvo == True:
            ...
            m_CLanParams.orig_frame = m_CLanParams.left_image.get_data()
            # Convert SVO image from RGBA to RGB
            m_CLanParams.orig_frame = cv2.cvtColor(m_CLanParams.orig_frame, cv2.COLOR_RGBA2RGB)
            m_CLanParams.orig_frame = cv2.cvtColor(m_CLanParams.orig_frame, cv2.COLOR_RGB2BGR)
            plt.figure('ZED SVO File')
            plt.title('Visible and Depth')        
            plt.imshow(m_CLanParams.orig_frame,cmap='gray')
            plt.show(block=False)
            plt.pause(0.5)
            # plt.close()           
            
            # RGBPiplines(orig_frame) 
            m_CFRGBLneTrcks.MainDeteTrack(m_CLanParams.orig_frame) 
